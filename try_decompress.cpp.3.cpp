...
size_t LZ4_uncompress_unknownOutputSize(const uint8_t* source, uint8_t* dest, int isize, int maxOutputSize, decompression_error& error)
{
	...
	while (ip < iend)
	{
		...
		if (op - ref < STEPSIZE) {
			...
			*reinterpret_cast<uint64_t*>(op) = *reinterpret_cast<const uint64_t*>(ref);
			op += sizeof(uint64_t);
			ref += sizeof(uint64_t);
		}
...
bool lz4_decompress(const uint8_t* const src, size_t src_size, std::vector<uint8_t>& out, decompression_error& error)
{
	...
	out.resize(999999);
	bool res = (LZ4_uncompress_unknownOutputSize(src + sizeof(uint32_t), out.data(), input_size, out.size(), error) != 0);
	return res;
}
